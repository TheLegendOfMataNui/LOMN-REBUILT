...
ifeq ($(windows),1)
	...
	@$(subst \,/,$(DLTOOL)) https://github.com/TheLegendOfMataNui/FMVs/releases/download/$(FMVVERSION)/$(notdir $@) $@
endif
# 
...
ifeq ($(windows),1)
	...
	mkdir -p $(dir $@)
	cp $< $@
	mkdir -p ./build/scripting/slks/shoefitter/onua
endif
...
ifeq ($(windows),1)
	...
	mkdir -p $(dir $@)
	wine $(subst \,/,$(LSSTOOL)) $(LSSARGS) ./script -o $@ 
endif
...
ifeq ($(windows),1)
	...
	@mkdir -p $(dir $@)
	@mkdir -p $(patsubst build/data/%.blk,./blockfiles/%,$@)
	@$(subst \,/,$(BLKTOOL)) --$(if $(wildcard $(patsubst build/data/%.blk,./blockfiles/%/.nocompress,$@)),decompressed,$(COMPRESSION)) -o $@ $(patsubst build/data/%.blk,./blockfiles/%,$@) > /dev/null
endif
